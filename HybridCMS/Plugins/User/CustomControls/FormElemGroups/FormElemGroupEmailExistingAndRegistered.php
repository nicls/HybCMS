...
/**
 * class FormElemGroupEmailExistingAndRegistered is the concrete implemenatation for an form 
 * Input-Field for a Email of an existing and registered user  for e.g. login
 *
 * @package CustomControls
 * @author Claas Kalwa
 * @copyright 2014 Claroweb.de
 */
class FormElemGroupEmailExistingAndRegistered extends
    \HybridCMS\Plugins\User\CustomControls\FormElemGroups\FormElemGroupEmail
{
    ...
    public function validateContent()
    {
        ...
        {                        
            ...
            else if(false === $this->objContent->emailExistsAndUserIsRegistered() )
            {            
                $this->setGroupHasWarning(true); 
                
                if(true === $this->objContent->emailExists())
                {            
                    $this->objContent->setWarningMsg('Die E-Mail Adresse muss zunächst bestätigt werden.');
                }                  
            }
            else
            {
