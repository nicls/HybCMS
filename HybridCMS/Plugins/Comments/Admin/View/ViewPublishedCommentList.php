...
/**
 * class ViewPublischedCommentsList
 *
 * @package View
 * @author Claas Kalwa
 * @copyright 2014 Claroweb.de
 */
class ViewPublishedCommentList extends 
...
{    
    ...
    {
        assert(true === isset($this->arrSettings['arrObjPubComments']));
        assert(true === isset($this->arrSettings['objButtonDeleteComment']));
        assert(true === isset($this->arrSettings['objButtonUnpublishComment']));
        
        //get comments
        $arrObjPubComments = $this->arrSettings['arrObjPubComments'];        
               
        $op = '';                
        
        //add user response on published Comments
        if(true === isset($this->arrSettings['commentUnpublished'])
           &&
           true === $this->arrSettings['commentUnpublished'] 
           && 
          ...
          )
        {
            ...
            $op .= '<p class="add-top-50 bg-success">Der Kommentar ist jetzt nicht mehr öffentlich.</p>';
            $op .= '</a>';
        }
        ...
        $op .= '<div class="panel-group" id="accordion-publishedComments">';
        $op .= '<div class="panel panel-default">';
        $op .= '<div class="panel-heading">';
        $op .= '<header class="panel-title">';
        $op .= '<a data-toggle="collapse" data-parent="#accordion-publishedComments" href="#collapseTwo">';
        $op .= count($arrObjPubComments) . ' veröffentlichte Lesermeinungen';
        $op .= '</a>';
        $op .= '</header>';
        $op .= '</div>';
        $op .= '<div id="collapseTwo" class="panel-collapse collapse">';
        $op .= '<div class="panel-body">';
                        
        foreach($arrObjPubComments as &$objCmnt)
        {
            ...
            //submitting the form
            $op .= $this->toStringFormElemGroup('objButtonUnpublishComment');            
